...
#' cell_features <- read.table(cell_features_file,
#'   sep = "\t", header = TRUE,
...
readDgeFile <- function(dgeMatrixFile, cell_features = NULL) {
  errMsg <- paste0(
    "The cell barcodes in the DGE matrix do not match the cell",
  ...
  if (!is.null(dgeMatrixFile)) {
    ...
    if (!is.null(cell_features)) {
      dgeMatrix <- dgeMatrix[, match(
        cell_features$cell_barcode,
        colnames(dgeMatrix)
      )]
...
parse10xMTX <- function(dgeMatrixFile) {
  ...
  if (mtxExists && featureExists && barcodeExists) {
    ...
    stop(
      "The directory does not contain the required files for ",
      "a 10X mtx file."
    )
  }
}
...
fastRead <- function(inFile, comment_regexp = NA, ...) {
  ...
  if (grepl("\\.gz$", inFile)) {
    ...
    if (!is.na(comment_regexp)) {
      unzip_command <- paste0(unzip_command, sprintf(
        " | egrep -v '%s'",
        comment_regexp
      ))
    }
...
version_above <- function(pkg, than) {
  as.logical(utils::compareVersion(
...
fastReadBigGz <- function(inFile, ...) {
  ...
  # Run gunzip properly
  retval <- system2("gunzip",
    args = c("-c", inFile), stdout = t,
    stderr = FALSE
  ) != 0
...
readCellFeatures <- function(
  ...
  if (verbose && "num_transcripts" %in% colnames(cell_features)) {
    message(
      nrow(cell_features), " rows in file (",
