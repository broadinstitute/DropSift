...
fastRead<-function(inFile, comment_regexp=NA, ...) {
    ...
    if (length(grep(".gz", inFile))==1) {
        ...
        if (version_above("data.table", "1.11.8")) {
            return(tryCatch(data.table::fread(cmd=unzip_command,data.table=TRUE, ...), error=function(e) fastReadBigGz(inFile, comment_regexp=comment_regexp, ...)))
        } else {
            return(tryCatch(data.table::fread(unzip_command,data.table=TRUE, ...), error=function(e) fastReadBigGz(inFile, comment_regexp=comment_regexp, ...)))
        }
        ...
        if (!is.na(comment_regexp)) {
            ...
            a <- data.table::fread(input, data.table=TRUE, ...)
        } else {
            a <- data.table::fread(input, data.table=TRUE, ...)
        }
        return(a)
...
fastReadBigGz <- function(inFile, ...) {
    ...
    # Run gunzip properly
    retval <- system2("gunzip", args = c("-c", inFile), stdout = t, stderr = FALSE) != 0
    if (retval != 0) {
...
read_dge_gz<-function(file, decreasing_order_by_size=TRUE) {
    ...
    if (decreasing_order_by_size) {
        ...
        order.data <- names(colsums.full.data)[order(colsums.full.data,decreasing=TRUE)]
